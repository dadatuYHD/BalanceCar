...
{	
	...
	{    
		...
		q2 = quat[2] / q30;
		q3 = quat[3] / q30;
		Pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3; //输出的是陀螺仪绕其规定的Y轴旋转的角度，它是陀螺仪的偏航角但由于陀螺仪安装，在实际上是小车的俯仰角PITCH
//        Roll = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3; // 输出的是绕陀螺仪X轴旋转的角度，是陀螺仪的俯仰角，由于小车安装，就是小车的横滚角ROLL	 
//		Yaw =  atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;//输出的是绕陀螺仪Z轴旋转的角度，是陀螺仪的翻滚角，由于实际安装，是小车偏航角		
	}
}
/**************************************************************************
