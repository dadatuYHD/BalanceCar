...
{  	 
	...
	MY_NVIC_Init(4,0,USART3_IRQn,3);//组2，最低优先级 
}
...
{	
	...
	{	  
		...
		else if (uart_receive == 0x42||uart_receive==0x43||uart_receive==0x44)	Flag_Qian=0,Flag_Hou=0,Flag_Left=0,Flag_Right=1;  //左
		else if (uart_receive==0x46 || uart_receive==0x47 || uart_receive==0x48) Flag_Qian = 0, Flag_Hou = 0, Flag_Left = 1, Flag_Right = 0;  //右   	
